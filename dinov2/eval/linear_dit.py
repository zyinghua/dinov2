...
def get_args_parser(
    ...
    parser.add_argument(
        ...
        type=str,
        default="DiT-XL/2",
        help="DiT model architecture name",
    )
    parser.add_argument(
        "--dit-image-size",
        type=int,
        default=256,
        help="Input image size for DiT",
    )
    parser.add_argument(
        "--dit-timestep",
        type=float,
        default=1.0,
        help="Timestep for DiT forward pass (default: 1.0)",
    )
    parser.set_defaults(
...
def create_linear_input_dit(x_tokens_list, use_n_blocks, use_avgpool):
    """
    Create linear input from DiT features using average pooling.
    
    Args:
        x_tokens_list: List of feature tensors from different blocks, each of shape (B, N_patches, hidden_dim)
        use_n_blocks: Number of blocks to use (from the last blocks)
        use_avgpool: Whether to use average pooling (always True for DiT, but kept for compatibility)
    
    Returns:
        Concatenated and pooled features of shape (B, feature_dim)
    """
    intermediate_output = x_tokens_list[-use_n_blocks:]
    
    # Average pool each block's patch tokens across sequence dimension (dim=1)
    pooled_features = [feat.mean(dim=1) for feat in intermediate_output]  # Each: (B, hidden_dim)
    
    # Concatenate features from multiple blocks
    output = torch.cat(pooled_features, dim=-1)  # (B, use_n_blocks * hidden_dim)
    
    return output.float()


class LinearClassifierDiT(nn.Module):
    """Linear layer to train on top of frozen DiT features"""
    ...
    def forward(self, x_tokens_list):
        output = create_linear_input_dit(x_tokens_list, self.use_n_blocks, self.use_avgpool)
...
class ModelWithIntermediateLayersDiT(nn.Module):
    """
    Wrapper to extract intermediate features from multiple DiT blocks.
    """
    def __init__(self, dit_feature_extractor, n_last_blocks):
        super().__init__()
        self.dit_feature_extractor = dit_feature_extractor
        self.n_last_blocks = n_last_blocks
        self.dit_feature_extractor.dit_model.eval()
        
    def forward(self, images):
        """
        Extract features from the last n_last_blocks of DiT.
        
        Args:
            images: Input images of shape (B, 3, H, W) in range [0, 1]
            
        Returns:
            List of feature tensors from the last n_last_blocks, each of shape (B, N_patches, hidden_dim)
        """
        with torch.inference_mode():
            # Extract features from multiple blocks
            features_list = self._extract_multiple_blocks(images)
        return features_list
    
    def _extract_multiple_blocks(self, images):
        """
        Extract features from the last n_last_blocks.
        ...
        latents = self.dit_feature_extractor.vae.encode(images_normalized).latent_dist.sample()
        latents = latents * 0.18215  # VAE scaling factor
        
        # Prepare timestep
        t = torch.full((batch_size,), self.dit_feature_extractor.dit_timestep, 
                      device=self.dit_feature_extractor.device, dtype=torch.float32)
        
        # Always use null class (num_classes) for unconditional CFG behavior
        y = torch.full((batch_size,), self.dit_feature_extractor.num_classes, 
                      dtype=torch.long, device=self.dit_feature_extractor.device)
        
        # Forward through DiT and extract from multiple blocks
        features_list = []
        
        # Patch embedding
        x = self.dit_feature_extractor.dit_model.x_embedder(latents) + self.dit_feature_extractor.dit_model.pos_embed
        
        # Timestep and class embedding
        t_embed = self.dit_feature_extractor.dit_model.t_embedder(t)
        y_embed = self.dit_feature_extractor.dit_model.y_embedder(y, train=False)
        c = t_embed + y_embed
        
        # Forward through blocks and collect features from last n_last_blocks
        num_blocks = len(self.dit_feature_extractor.dit_model.blocks)
        start_block = num_blocks - self.n_last_blocks
        
        for i, block in enumerate(self.dit_feature_extractor.dit_model.blocks):
            x = block(x, c)
            if i >= start_block:
...
def setup_linear_classifiers(sample_output, n_last_blocks_list, learning_rates, batch_size, num_classes=1000):
    ...
    for n in n_last_blocks_list:
        for avgpool in [True]:  # Always use avgpool for DiT (no CLS token)
            for _lr in learning_rates:
                lr = scale_lr(_lr, batch_size)
                out_dim = create_linear_input_dit(sample_output, use_n_blocks=n, use_avgpool=avgpool).shape[1]
                linear_classifier = LinearClassifierDiT(
...
def run_eval_linear(
    ...
    dit_feature_extractor = DiTFeatureExtractor(
    ...
    # Create feature model wrapper
    feature_model = ModelWithIntermediateLayersDiT(dit_feature_extractor, n_last_blocks)
    
    # Get sample output to determine feature dimensions
    sample_image = train_dataset[0][0].unsqueeze(0).cuda()
...
def main(args):
    # Note: We don't use setup_and_build_model for DiT, we use DiTFeatureExtractor instead
    run_eval_linear(
        dit_model_path=args.dit_model_path,
        dit_model_name=args.dit_model_name,
        dit_image_size=args.dit_image_size,
...
if __name__ == "__main__":
    description = "DiT linear evaluation"
