...
The `forward` function by the DiT model will return a list of intermediate DiT features when `capture_intermediates` is set to True. For more information please look at the `forward` function of the `DiT` class in the `models.py` file. You can also play around with the `run_DiT.ipynb` to get a sense of the intermediate features.
